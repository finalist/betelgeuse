...
public class GAMA3DHandler implements MeshFormatHandler {
    ...
    public void write(OutputStream os, SimpleVFS vfs) throws Exception {
        ...
        for (int i = 0; i < mesh.vertices.length; i++)
            dos.writeFloat(mesh.vertices[i]);

        for (int i = 0; i < mesh.faces.length; i++)
            dos.writeShort(mesh.faces[i]);

        for (int i = 0; i < mesh.colors.length; i++)
            dos.writeInt(mesh.colors[i]);

        for (int i = 0; i < mesh.faceColors.length; i++)
            dos.writeShort(mesh.faceColors[i]);
    }
