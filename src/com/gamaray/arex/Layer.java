...
public class Layer 
{
	...
	{
		if (!xmlHasRefreshDistance) return false;
	...
	{
		if (!xmlHasRefreshTime) return false;
	...
	public void load(Element root) throws Exception
	{
		...
		if (xmlVersion == null) throw new Exception("Dimension does not specify a version");
	...
	{	
		...
		if (soundElem != null) xmlSoundUrl = soundElem.getChildElementValue("url");
		...
		{
			xmlValidWithinRange = Float.parseFloat(refreshDistanceElem.getChildElementValue("validWithinRange", "1000"));
		...
		if (assetsElem != null)
		{
			for (int i = 0; i < assetsElem.childElements.size(); i++)
			{
				...
				if (assetElem.name.equals("asset"))
				{
					...
					if (das.xmlUrl == null) throw new Exception("No URL in asset '" +  das.xmlId + "'");
			...
			for (int i = 0; i < placemarksElem.childElements.size(); i++)
			{
				...
				if (pm != null)
				{
					...
					if (pm.xmlLocationId == null)
					{
						...
						if (locElem == null) throw new Exception("No location in feature '" +  pm.xmlId + "'");
					...
					else if (pm instanceof PlacemarkTxt)
					{
						...
						pmTxt.xmlShowInRadar = Boolean.parseBoolean(pmElem.getChildElementValue("showInRadar", "false"));
					}
	...
	{
		if (!idMap.containsKey(id)) throw new Exception("Id '" + id + "' not found, requested by '" + requestorId + "'");
	}
	
	private void putId(String id) throws Exception
	{
		if (idMap.containsKey(id)) throw new Exception("Duplicate id '" + id + "'");
		idMap.put(id, "VALUE");
	}
}
