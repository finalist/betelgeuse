...
public class ARXView 
{
	...
	{	
		...
		if (!ctx.gpsEnabled())
		{
			ARXMessages.putMessage("GPS_ENABLED", 
				"GPS not Enabled (see Settings->Security & location->Enable GPS)", ARXMessages.errorIcon, 500);
		}
		
		// Check if GPS signal received
		if (!ctx.gpsSignalReceived())
		{
			ARXMessages.putMessage("GPS_WAIT", 
				"Waiting for GPS signal", ARXMessages.worldIcon, 500);
		}
		
		// Check for compass accuracy
		if (ctx.isCompassAccuracyLow())
		{
			ARXMessages.putMessage("COMPASS_ACCURACY", 
				"Compass accuracy low", ARXMessages.compassIcon, 1000);
		}	
	...
	{	
				...
				(state.nextLayerStatus == ARXState.READY && state.launchNeeded))
		{
			...
			if (!state.launchUrl.equals("")) 
				request.params = state.getParams("init","NULL") + "&url=" + state.launchUrl;
			else
		...
		{
			ARXMessages.putMessage("LOADING_" + state.downloadJobId, 
				"Dimension loading", ARXMessages.refreshIcon, 500);
				...
				if (state.downloadResult.error)
				{
					...
					{	
						...
						ARXMessages.putMessage("ERROR_" + state.downloadJobId, 
							"Error: " + state.downloadResult.errorMsg, ARXMessages.warningIcon, Integer.MAX_VALUE);
						
						ARXMessages.putMessage("LOADING_" + state.downloadJobId, 
							"Dimension loading (retry #" + state.retryCount + ")", ARXMessages.refreshIcon, Integer.MAX_VALUE);
					}
					else
					{
						...
						ARXMessages.putMessage("FATAL_ERROR_" + state.downloadJobId, 
							"Unable to load dimension: " + state.downloadResult.errorRequest.url, ARXMessages.errorIcon, Integer.MAX_VALUE);
					}
				}
				else
				{
					...
					//ARXMessages.putMessage("LOADING_COMPLETE_" + state.downloadJobId, 
					//	"Dimension '" + newLayer.xmlName + "' loaded", ARXMessages.refreshCompleteIcon, 3000);
					...
					for (int i = 0; i < state.layer.placemarks.size(); i++)
					{
						...
						if (pm instanceof Placemark3D)
						{
							...
							if (dest != null) src.copyRenderState(dest);
						}
					}
					...
					for (int i = 0; i < state.layer.assets.size(); i++)
					{
						...
						if (dest != null) das.copyDownloadState(dest);
					}
					...
					if (newLayer.xmlRadarAvailable) state.radarVisible = true;
					if (newLayer.xmlHasRadarRange) radarDialog.setRange(newLayer.xmlRadarRange);
		...
		for (int i = 0; i < state.layer.assets.size(); i++)
		{
			...
			if (das.downloadStatus == ARXState.JOB_SUBMITTED)
			{
				...
				if (das.downloadJobId.equals(activeJobId))
				{
					...
					ARXMessages.putMessage("DOWNLOAD_ASSET", 
						"Downloading '" + das.xmlId + "' (" + pctTxt + "%)", "[" + pctTxt + "%]",
		...
		if (state.nextLayerStatus == ARXState.READY)
		{
			...
			for (int i = 0; i < state.layer.assets.size(); i++)
			{
				...
				else if (das.downloadStatus == ARXState.JOB_SUBMITTED)
				{
					if (ctx.getARXDownload().isJobComplete(das.downloadJobId))
					{
						...
						{
							ARXMessages.putMessage("DOWNLOAD_ASSET", 
								"Download of '" + das.xmlId + "' complete", "",
								ARXMessages.downloadCompleteIcon, 500, true);
						}
						else
						{
							ARXMessages.putMessage("DOWNLOAD_" + das.downloadJobId, 
								"Error downloading '" + das.xmlId + "'", "", 
					...
					else
					{
						...
						if (das.downloadJobId.equals(activeJobId))
						{
							...
							String pctTxt = ARXUtil.formatDecimal(das.downloadJobPctComplete*100, 2);
							ARXMessages.putMessage("DOWNLOAD_ASSET", 
								"Downloading '" + das.xmlId + "' (" + ARXUtil.formatDecimal(das.downloadJobPctComplete*100, 2) + "%)", 
								"[" + pctTxt + "%]",
								ARXMessages.downloadIcon, 500, true);
						}
					}
			...
			if (allAssetsDownloaded && !state.layer.allAssetsDownloaded)
			{
				...
				ARXMessages.putMessage("ASSETS_LOADING_COMPLETE", 
					"Dimension '" + state.layer.xmlName + "' loaded", "", ARXMessages.refreshCompleteIcon, 3000, true);
			}
			
			if (!allAssetsDownloaded)
			{
				ARXMessages.putMessage("ASSETS_LOADING", 
					"Dimension '" + state.layer.xmlName + "' loading", ARXMessages.refreshIcon, 500);
			}
		}
		...
		if (state.radarVisible && state.layer.xmlRadarAvailable)
		{
			...
			dw.drawLine(
				leftRadarLine.x, leftRadarLine.y, 
				radarX+RadarObjects.RADIUS, radarY+RadarObjects.RADIUS);
			dw.drawLine(
				rightRadarLine.x, rightRadarLine.y, 
				radarX+RadarObjects.RADIUS, radarY+RadarObjects.RADIUS);
			dw.setColor(Color.rgb(255,255,255));
			dw.setFontSize(12);
			drawRadarTextLine(dw, ARXUtil.formatDistance(radarDialog.getRange()), radarX + RadarObjects.RADIUS, radarY + RadarObjects.RADIUS*2 + 5);
		...
		{
			String evtName = null;
			if (state.layer.refreshOnDistance(state.curFix)) evtName = "refreshOnDistance";
			if (state.layer.refreshOnTime(System.currentTimeMillis())) evtName = "refreshOnTime";
		...
		if (dialogState == INFO_DIALOG)
		{
			...
			float infoX = infoPad, infoY = infoPad, infoW = width - infoPad * 2, infoH = infoLineHeight * 10 + infoInnerPad * 2;
			...
			String validRangeTxt = (state.layer.xmlHasRefreshDistance) ? ARXUtil.formatDistance(state.layer.xmlValidWithinRange) : "NA";
		...
		else if (dialogState == RADAR_DIALOG)
		{
			...
			{
				ClickEvent radarEvt = new ClickEvent(((ClickEvent) evt).x-radarDialogX, ((ClickEvent) evt).y-radarDialogY);
				
				radarDialog.doAccept = false;
				radarDialog.handleEvent(radarEvt);
				if (radarDialog.doAccept) dialogState = NO_DIALOG;
			}
		}
		else
		{
	...
	boolean handleClickEvent(ClickEvent evt)
	{
		...
		if (state.radarVisible && state.layer.xmlRadarAvailable)
		{
			if (evt.x > radarX && evt.x < radarX + radarObjects.getWidth() &&
				evt.y > radarY && evt.y < radarY + radarObjects.getHeight())
			{
...
class RadarDialog implements Drawable
{
	...
	public boolean handleEvent(ClickEvent event)
	{
		...
		else if (ex > sliderBarX-2 && ex < sliderBarX + sliderBarWidth && ey > sliderBarY && ey < sliderBarY + sliderBarHeight)
		{
			// move slider
			...
			if (sliderVal > sliderRange) sliderVal = sliderRange;
		}
