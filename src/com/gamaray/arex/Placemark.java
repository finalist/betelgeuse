...
abstract public class Placemark
{
	...
	static int blobOuterColor = Color.rgb(0,0,255),  blobInnerColor = Color.rgb(100,100,255),  blobWorkColor = Color.rgb(50,50,255);  
...
class Placemark3D extends Placemark
{
	...
	void updateState(GeoPoint curGPSFix, long time)
	{
		...
		if (layer.xmlRelativeAltitude) obj.location.y = (float) geoLoc.alt;
		...
		if (!stale && drawMode == RENDER_READY)
		{
			...
			if (dot < angleThreshold || ratio > 0.1) stale = true;
		}
	}
	...
	{	
		if (isVisible)
		{
			if (drawMode == RENDER_READY)
			{
				float renderAngle = ARXUtil.getAngle(
					renderResult.centerX, renderResult.centerY, 
					renderResult.signPostX, renderResult.signPostY);
				float currentAngle = ARXUtil.getAngle(
					centerMark.x, centerMark.y, 
					signPostMark.x, signPostMark.y);
				
        		dw.drawBitmap(renderResult.cam.buf,
        			centerMark.x - renderResult.centerX, centerMark.y - renderResult.centerY, 
        			renderResult.cam.width, renderResult.cam.height,
        			currentAngle - renderAngle,
        			renderResult.scaleFactor);
	...
	{
		if (!isCenterVisible || !stale) return;
	...
	boolean transformEqual(Placemark3D dest)
	{
		...
		return (dest.xmlScale == src.xmlScale && 
			dest.xmlRotX == src.xmlRotX && 
			dest.xmlRotY == src.xmlRotY && 
			dest.xmlRotZ == src.xmlRotZ);
	}
}
		...
		if (isVisible)
		{
			if (asset.downloadStatus == ARXState.READY)
			{
				...
				else
				{
					...
					dw.drawObject(anchoredObj, 
						centerMark.x - anchoredObj.getWidth()/2, centerMark.y - anchoredObj.getHeight()/2, 
						currentAngle + 90, 1);
