...
class ARXUtil 
{
	...
	public static boolean rectTriIntersect(
		float r_x, float r_y, float r_w, float r_h,
		float A_x, float A_y, float B_x, float B_y, float C_x, float C_y)
	{
		float rp1_x, rp1_y, rp2_x, rp2_y, rp3_x, rp3_y, rp4_x, rp4_y;
		
		// Is any point on triangle inside rectangle (CCL)
		if (A_x > r_x && A_x < r_x + r_w && A_y > r_y && A_y < r_y + r_h) return true;
		if (B_x > r_x && B_x < r_x + r_w && B_y > r_y && B_y < r_y + r_h) return true;
		if (C_x > r_x && C_x < r_x + r_w && C_y > r_y && C_y < r_y + r_h) return true;
		
		// Are all points on triangle outside one edge of rectangle (CCL)
		if (A_x < r_x && B_x < r_x && C_x < r_x) return false;
		if (A_y < r_y && B_y < r_y && C_y < r_y) return false;
		if (A_x > r_x + r_w && B_x > r_x + r_w && C_x > r_x + r_w) return false;
		if (A_y > r_y + r_h && B_y > r_y + r_h && C_y > r_y + r_h) return false;
		...
		if (pointInTriangle(rp1_x, rp1_y, A_x, A_y, B_x, B_y, C_x, C_y)) return true;
		if (pointInTriangle(rp2_x, rp2_y, A_x, A_y, B_x, B_y, C_x, C_y)) return true;
		if (pointInTriangle(rp3_x, rp3_y, A_x, A_y, B_x, B_y, C_x, C_y)) return true;
		if (pointInTriangle(rp4_x, rp4_y, A_x, A_y, B_x, B_y, C_x, C_y)) return true;
		
		// Do any lines intersect
		if (lineIntersect(rp1_x, rp1_y, rp2_x, rp2_y, A_x, A_y, B_x, B_y)) return true;
		if (lineIntersect(rp2_x, rp2_y, rp3_x, rp3_y, A_x, A_y, B_x, B_y)) return true;
		if (lineIntersect(rp3_x, rp3_y, rp4_x, rp4_y, A_x, A_y, B_x, B_y)) return true;
		if (lineIntersect(rp4_x, rp4_y, rp1_x, rp1_y, A_x, A_y, B_x, B_y)) return true;
		
		if (lineIntersect(rp1_x, rp1_y, rp2_x, rp2_y, B_x, B_y, C_x, C_y)) return true;
		if (lineIntersect(rp2_x, rp2_y, rp3_x, rp3_y, B_x, B_y, C_x, C_y)) return true;
		if (lineIntersect(rp3_x, rp3_y, rp4_x, rp4_y, B_x, B_y, C_x, C_y)) return true;
		if (lineIntersect(rp4_x, rp4_y, rp1_x, rp1_y, B_x, B_y, C_x, C_y)) return true;
		
		if (lineIntersect(rp1_x, rp1_y, rp2_x, rp2_y, C_x, C_y, A_x, A_y)) return true;
		if (lineIntersect(rp2_x, rp2_y, rp3_x, rp3_y, C_x, C_y, A_x, A_y)) return true;
		if (lineIntersect(rp3_x, rp3_y, rp4_x, rp4_y, C_x, C_y, A_x, A_y)) return true;
		if (lineIntersect(rp4_x, rp4_y, rp1_x, rp1_y, C_x, C_y, A_x, A_y)) return true;
		
		return false;
	}
	
	public static boolean lineIntersect(
		float begin__x_, float begin__y_, float end__x_, float end__y_,
	...
	public static void fillRect(int buf[], int bufWidth, int bufHeight, int rx, int ry, int width, int height, int color)
	{
		for (int y = 0; y < height; y++)
		{
			for (int x = 0; x < width; x++)
			{
				...
				if (bufIdx >= 0 && bufIdx < buf.length) buf[bufIdx] = color;
			}
		}
	}
