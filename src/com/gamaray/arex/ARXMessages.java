...
public class ARXMessages 
{
	public static Bitmap 
		refreshIcon, refreshCompleteIcon,
		targetIcon, compassIcon, worldIcon,
	...
	public static void putMessage(String msgId, String msgTxt, String miniTxt, Bitmap icon, long duration, boolean volatileMsg)
	{
		...
		for (int i = 0; i < messages.size(); i++)
		{
			...
			{
				if (msgTxt != null) msg.msgTxt = msgTxt;
				if (miniTxt != null) msg.miniTxt = miniTxt;
				if (icon != null) msg.icon = icon;
				if (duration != -1) msg.duration = duration;
				if (duration != -1) msg.expires = System.currentTimeMillis() + msg.duration;
		...
		if (!updated)
		{
			...
			if (msgId == null) msgId = "AUTO_ID_" + (count++);
			newMsg.msgId = msgId;
	...
	public static void removeMessage(String msgId)
	{
		for (int i = 0; i < messages.size(); i++)
		{
			...
			if (msg.msgId.equals(msgId)) msg.expires = 0;
		}
	}
...
class Message implements Drawable
{
	...
	public boolean isImportant()
	{
		if (icon == ARXMessages.refreshIcon || 
			icon == ARXMessages.refreshCompleteIcon || 
			icon == ARXMessages.downloadErrorIcon || 
			icon == ARXMessages.errorIcon || 
			icon == ARXMessages.warningIcon ||
			icon == ARXMessages.worldIcon)
		{
			return true;
		}
		else
	...
	public void init(DrawWindow dw, float maxWidth)
	{
		...
		tb = new TextBlock(
			msgTxt, 12, maxWidth - icon.getWidth() - pad*3, 
			Color.rgb(255,255,255), Color.rgb(255,255,255), Color.rgb(0,0,0), 0, dw);
	...
	public void draw(DrawWindow dw)
	{
		if (fullMessage)
		{
			...
			{
				if (step != 0) dw.drawBitmap(icon, pad, miniHeight/2 - icon.getHeight()/2, 0, 1);
			}
			else
			{
...
class VolatileMessage extends Message
{
	...
	public void draw(DrawWindow dw)
	{
		...
		{
			dw.drawBitmap(icon, pad, miniHeight/2 - icon.getHeight()/2, 0, 1);
			//dw.drawText(icon.getWidth() + pad*2, dw.getTextAscent()+pad, miniTxt);
		}
	}
}
